...
GUASSIAN_WIDTH = {"charades":   { "30": 1.5, "20": 3.0, "10": 4.0, "5":  5.0},
                  "anet":       { "30": 1.5, "20": 3.0, "10": 4.0, "5":  5.0},
                  "tacos":       { "30": 1.5, "20": 3.0, "10": 4.0, "5":  5.0},
                 }
...
def get_props_nms(sprob, eprob, topk):
    ...
    matrix = np.matmul(sprob, eprob)
    matrix = np.triu(matrix, k=0)
    res = nms_matrix(matrix, topk=topk, thresh=0.5)
    return res


def get_candidate_label(sprob, eprob, sbmn, ebmn, vlen):
    # res = get_props_nms(sprob, eprob, 10)

    if sbmn is None:
        res = get_props_nms(sprob, eprob, 10)
    else:
        res = get_props_nms(sprob*sbmn, eprob*ebmn, 10)
    tmp = res[:10].tolist()
    rr = []
    for i in tmp:
